...
import (
	...
	bc "github.com/libonomy/libonomy-p2p/config"
	"github.com/libonomy/libonomy-p2p/filesystem"
...
func setupLogging(config *bc.Config) {
	...
	if err != nil {
		fmt.Printf("Failed to setup libonomy data dir")
		log.Panic("Failed to setup libonomy data dir", err)
	}

	// app-level logging
	log.InitlibonomyLoggingSystem(config.DataDir, "libonomy.log")
}
